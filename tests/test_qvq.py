...
from core.models.qwenvl import batch_generate_with_vllm
from core.models.qwen import batch_generate

TEST_NAME = 'qvq72b_once'
VLM_PATH = os.getenv('VLM_PATH_QVQ_INFER')
...
def test_a_path(
    ...
    if 'twice' in TEST_NAME:
        ...
        raise ValueError("TEST_NAME must contain 'twice' or 'once' to determine the grounding method.")
    
    json_responses = batch_infer_pipeline(
        images, questions, VLM_PATH,
        batch_infer_func=batch_generate_with_vllm,
        max_tokens=1024
    )
