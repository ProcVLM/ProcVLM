"""
CUDA_VISIBLE_DEVICES=0,1 python ecot/run_done.py --world_size 4 --rank 0
CUDA_VISIBLE_DEVICES=2,3 python ecot/run_done.py --world_size 4 --rank 1
CUDA_VISIBLE_DEVICES=4,5 python ecot/run_done.py --world_size 4 --rank 2
CUDA_VISIBLE_DEVICES=6,7 python ecot/run_done.py --world_size 4 --rank 3
"""
...
# ===== config =====
BATCH_SIZE = 1 # needs update #
TENSOR_PARALLEL_SIZE = 2
VLM_PATH = os.getenv('VLM_PATH_INTERNVL_INFER')
DONE_JSONL_OUTPUT_DIR = os.getenv('OUTPUT_DONE_JSONL_DIR')
SUB_TASK_JSONL_DIR = os.getenv('ANNOTATION_SUB_TASK_DIR')
BBOX_JSONL_DIR = os.getenv('ANNOTATION_BBOX_JSONL_DIR')
...
def producer_task(input_queue: mp.Queue, dataset_path: str, rank: int, world_size: int, refresh_freq: int):
    ...
    for _tidx, ep_stem in enumerate(tqdm(eps, desc=f"[{dataset.name}] SD R{rank}/{world_size}", total=total_frames)):
        ...
        for sti, (start_fid, sub_task) in enumerate(subtask_borders[:-1]):
            ...
            slen = max(1, int(0.25 * (end_fid - start_fid)))
            for fid in range(end_fid - slen, end_fid):
                # --- skip already done ---
