"""
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python ecot/run_done_qwen_task.py --world_size 2 --rank 0
"""
...
def producer_task(input_queue: mp.Queue, dataset_path: str, rank: int, world_size: int, refresh_freq: int):
    ...
    for _tidx, task_start in enumerate(tqdm(task_borders[:-1], desc=f"[{dataset.name}] SD R{rank}/{world_size}", total=total_frames)):
        ...
        if not task:
            logging.warning(f"Episode '{ep_stem}' task_id {task_id} has empty task description, skipping.")
            continue

        data1 = dataset[task_end - 1]
        start_fid, end_fid = int(data0['frame_index']), int(data1['frame_index'])
        ...
        if dej.exists():
            ...
            for ditem in done_items:
                possible_ep_rid.add((ditem['start_frame_id'], ditem['end_frame_id']))

        # --- cache for .bbox.jsonl ---
        # bxj = Path(BBOX_JSONL_DIR) / dataset.name / f"{ep_stem}.bbox.jsonl"
        # bbox_items = {}
        # if bxj.exists():
        #     loaded_bxj = load_jsonlines(bxj)
        #     for bitem in loaded_bxj:
        #         bbox_items[(bitem['frame_id'], bitem['camera_key'])] = bitem['bboxes']
        # else:
        #     logging.warning(f"Episode '{ep_stem}' has no bbox annotation file, images may miss objects of interest.")

        # --- skip already done ---
        if (start_fid, end_fid) in possible_ep_rid:
            continue

        prompt = prompt_example.format(instruction=task)
        
        # def try_enhance_image(img, frame_id, camera_key):
        #     jr = bbox_items.get((frame_id, camera_key), [])
        #     if jr: img = draw_bboxes(img, json_response_to_bboxes(jr))
        #     return img
        # image0 = try_enhance_image(data0['exo_image'], start_fid, dataset.exo_camera_key)
        # image1 = try_enhance_image(data1['exo_image'], end_fid, dataset.exo_camera_key)
        # image_list = [image0, image1]

        image_list = [data0['exo_image'], data1['exo_image']]

        # --- submit to input queue ---
        _supp = (start_fid, end_fid, task, ep_stem)
