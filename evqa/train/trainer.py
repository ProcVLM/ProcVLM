...
_ATTENTION_PATCH_BACKUP = {}

def apply_patch(module, name, new_val):
    key = (module, name)
    if key not in _ATTENTION_PATCH_BACKUP:
        _ATTENTION_PATCH_BACKUP[key] = getattr(module, name)
    setattr(module, name, new_val)


def replace_qwen2_vl_attention_class():
    import transformers.models.qwen2_vl.modeling_qwen2_vl as q2_vl
    import transformers.models.qwen2_5_vl.modeling_qwen2_5_vl as q2_5_vl
    import transformers.models.qwen3_vl.modeling_qwen3_vl as q3_vl
    import transformers.models.qwen3_vl_moe.modeling_qwen3_vl_moe as q3_vl_moe

    # --- Qwen2 VL ---
    apply_patch(q2_vl.Qwen2VLAttention, "forward", qwen2vl_forward)
    apply_patch(q2_vl, "create_causal_mask", return_mask)
    apply_patch(q2_vl, "create_sliding_window_causal_mask", return_mask)

    # --- Qwen2.5 VL ---
    apply_patch(q2_5_vl.Qwen2_5_VLAttention, "forward", qwen2vl_forward)
    apply_patch(q2_5_vl, "create_causal_mask", return_mask)
    apply_patch(q2_5_vl, "create_sliding_window_causal_mask", return_mask)

    # --- Qwen3 VL ---
    apply_patch(q3_vl.Qwen3VLTextAttention, "forward", qwen3vl_forward)
    apply_patch(q3_vl, "create_causal_mask", return_mask)

    # --- Qwen3 VL MoE ---
    apply_patch(q3_vl_moe.Qwen3VLMoeTextAttention, "forward", qwen3vl_forward)
    apply_patch(q3_vl_moe, "create_causal_mask", return_mask)


def restore_qwen_attention_class():
    if not _ATTENTION_PATCH_BACKUP:
        print("No patches to restore.")
        return

    for (module, name), original_val in _ATTENTION_PATCH_BACKUP.items():
        setattr(module, name, original_val)
